...
const CURVES = {
  ...
  //
  // Every one of the twelve `c:0` keyframes in the shipped data sits on an `o` lane on pup_l/pup_r,
  // and channelsFor now ignores those lanes outright — the reference does not treat them as alpha.
  // So this curve currently governs nothing that renders, and the reading is kept because it is the
  // one the data supports: a step that holds until the next keyframe, not one that jumps ahead.
  //
  // It was read as a jump to the target for a long time, on the strength of a measurement that had
  // the wrong sign rather than poor precision. That measurement argued source-hold left the closed
  // frame at 24.2% of open area — 8762px against the ~8567px of two pupils — and concluded the
  // pupils never left. But `c: "000000"` is a HOLE punched with destination-out (see drawNode), so a
  // pupil SUBTRACTS from total ink: the comparison could not detect a pupil in either direction.
  0: () => 0,
...
// The pupils, whose `o` lane is ignored — see channelsFor for the measurement that says why.
const PUPIL_NODE = /^pup_/;
...
// THE VERTICAL LOOK. Separate from the turn: it has no per-eye sign — both eyes do the same thing —
// and it is much stronger than an earlier fit suggested.
//
// That earlier fit was taken over a sweep of +-250px, because the reference's own demo page puts its
// canvas 300px down a 900px viewport and there is no more room below it. 250px is a look of 0.278,
// where the height has barely moved (0.975 of rest), so the fitted response came out about ten times
// too weak and the eyes stayed almost flat when the pointer went high or low. Re-measured with the
// canvas moved to the middle of a 1400px viewport, so the pointer can travel as far vertically as it
// does horizontally:
//
//     our look[1]   -0.667  -0.444  -0.278   0   +0.278  +0.444  +0.667
//     height ratio   0.796   0.926   0.975  1.0   0.985   0.944   0.815
//     width  ratio   0.986   0.993   0.993  1.0   1.003   1.014   1.028
//     gap    ratio   0.987   0.990   0.993  1.0   1.003   1.013   1.026
//
// So the eyes SQUASH vertically — to 0.8 of their height at the extremes, against the 0.975 the old
// sweep saw — and at the same time widen slightly and, looking down, drift apart. Each channel is a
// quadratic in |look[1]| with its own coefficient per direction; all fit within 2%.
// The height pair is fitted against the RENDERED result, not against the reference's ratios directly.
// Taking the ratios straight (-0.441 / -0.389) overshot by about a factor of two — 91px tall where the
// reference measures 129 — because this factor multiplies the turn's own height law rather than
// replacing it, and the eyes are already slightly shorter than rest before it applies. These values
// are what the drawn heights actually match.
const LIFT_H_UP = -0.2037;
const LIFT_H_DOWN = -0.1667;
const LIFT_W_UP = -0.0338;
const LIFT_W_DOWN = 0.0632;
// Scaled down from the raw ratios for the same reason as the height pair: measured on the drawn
// result, the spacing moved 5px where the reference moves 1.5 looking up, and 10 where it moves 4.5
// looking down.
const LIFT_GAP_UP = -0.0104;
const LIFT_GAP_DOWN = 0.027;

// The look value the vertical fit treats as full deflection. Not the same as the turn's 0.554: the
// pair is fully turned horizontally by then, while the vertical squash is still deepening at 0.667,
// which is as far as the sweep could reach. Sharing one scale between the axes would saturate the
// vertical response at a third of its range.
const LIFT_LOOK_MAX = 0.667;
...
export class LarkRuntime {
  ...
  channelsFor(name, now) {
    ...
    for (const inst of this.active) {
      ...
      for (let i = 0; i < lanes.length; i += 2) {
        ...
        // The pupils' `o` lane is NOT literal alpha, and honouring it as alpha is what left an open
        // eye with no pupil in it.
        //
        // Measured on the reference over 900 frames and 27 seconds: not once does it show a lid more
        // than 90% open with the pupil under half its resting size. Through a blink the pupil holds
        // 26-42% of the eye's ink and only disappears once the lid is under ~27% open — which is
        // precisely what clipping a fixed pupil to a closing lid produces, and nothing like an alpha
        // ramp. The lane's own timings say the same thing: in blink5 it holds 0 until t=1033 while
        // the lid is fully back by ~1020, so read as alpha it must show a bare eye.
        //
        // Dropping it takes the "open lid, missing pupil" window from 180ms to 0 across all five
        // blink clips, and the pupil still vanishes at each closed frame — 0% of ink — because the
        // lid clip alone removes it. The lane presumably drives something the web build does not
        // render; whatever it is, alpha is not it.
        if (head.keypath === 'o' && PUPIL_NODE.test(name)) continue;
  ...
  applyTurn(ctx, box, s, piv, lift = 1, scaleWidth = true, liftW = 1) {
    if (!s && lift === 1) return;
    const u = clampUnit(s);
    const p = piv || [0.5, 0.5];
  ...
  drawNode(ctx, name, now, inheritedAlpha = 1) {
    ...
    if (look[0] || look[1]) {
      ...
      if (llSource && !o.ll) {
        ...
        // The vertical look: both eyes squash the same way, no per-eye sign. Fitted against the RAW
        // look rather than the turn's deflection scale, because the two axes reach their extremes at
        // different look values — the pair is fully turned by about 0.554 horizontally, while the
        // vertical response is still growing at 0.667. Sharing TURN_GAIN here would saturate the
        // vertical squash at a third of the way up.
        const ly = Math.max(-LIFT_LOOK_MAX, Math.min(LIFT_LOOK_MAX, look[1])) / LIFT_LOOK_MAX;
        const lm = ly * ly;
        const down = ly > 0;
        const lift = 1 + (down ? LIFT_H_DOWN : LIFT_H_UP) * lm;
        const liftW = 1 + (down ? LIFT_W_DOWN : LIFT_W_UP) * lm;
        const liftGap = 1 + (down ? LIFT_GAP_DOWN : LIFT_GAP_UP) * lm;
        ...
        const offset = box.x + box.w / 2 - this.pairCentreX();
        ctx.translate(-offset * pull + offset * (liftGap - 1), 0);
      }
    }
