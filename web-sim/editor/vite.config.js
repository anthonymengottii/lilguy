...
//
// It reaches OUT for exactly two things, both read-only:
//
//   ../lark.js                          the real runtime, imported not reimplemented
//   ../../../lilguy-fork/public/*.json  the site's own data, the same files tools/lark_pack.py reads
//
// `fs.allow` is what lets a dev server rooted here read those paths at all.
export default defineConfig({
  plugins: [react()],
  server: {
    port: 8795,          // 8793 serves the reference, 8794 serves web-sim; this is the next one
    fs: { allow: [HERE, WEB_SIM, REPO] },
  },
  resolve: {
    alias: {
      '@lark': path.join(WEB_SIM, 'lark.js'),
      '@data': path.join(REPO, 'lilguy-fork/public'),
    },
  },
});
